...
verify_pkgbuild() { # $1=AUR pkg name
  ...
  case "${fetch_mode:-plain}" in
    plain)    report_add "item_pkgb_source" "PASS" "source_plain" ;;
    snapshot) report_add "item_pkgb_source" "PASS" "source_snapshot"; [ "${STRICT:-0}" = "1" ] && report_add "item_plain_availability" "FAIL" "plain_missing_fail" || report_add "item_plain_availability" "WARN" "plain_missing_warn" ;;
    git)      report_add "item_pkgb_source" "PASS" "source_git";       [ "${STRICT:-0}" = "1" ] && report_add "item_plain_availability" "FAIL" "plain_missing_fail" || report_add "item_plain_availability" "WARN" "plain_missing_warn" ;;
  esac
...
aur_checkout_to() { # $1=pkg $2=workdir -> prints "<dir> <mode>" where mode=plain|snapshot|git
  ...
  log_info "Plain/snapshot failed; cloning AUR: $pkg"; git clone --depth=1 "https://aur.archlinux.org/${pkg}.git" "$workdir/$pkg" >/dev/null 2>&1 || die "Failed to fetch AUR via plain/snapshot and git clone for '$pkg'."; [ -f "$workdir/$pkg/PKGBUILD" ] || die "PKGBUILD not found after git clone at '$workdir/$pkg/PKGBUILD' (pkg=$pkg)."; printf '%s\tgit\n' "$workdir/$pkg"
}
