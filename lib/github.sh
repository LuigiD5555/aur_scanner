#!/usr/bin/env bash
# github.sh — GitHub URL parsing and README scraping
...
# Extra-safe repo extraction in case of unusual URLs; never empty.
safe_repo_from_url() {
  local url="$1" repo
...
build_candidates_from_github() { # $1=url
  ...
  base=()
  [ -n "${kebab:-}" ] && base+=("$kebab")
  # Always include repo-derived candidate (normalized)
  local repo_kebab
  repo_kebab="$(normalize_kebab "$repo")"
  [ -n "$repo_kebab" ] && base+=("$repo_kebab")
